...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .emede-property-row {
    display: grid;
    grid-template-columns: 1.35rem minmax(5.5rem, 8rem) minmax(0, 1fr);
    align-items: center;
    gap: 0.55rem;
    min-height: 2.15rem;
    padding: 0.2rem 0;
  ...
  .emede-property-value {
    min-width: 0;
    overflow-wrap: anywhere;
    font-size: 0.86em;
  }
  .emede-property-chips {
    display: flex;
    flex-wrap: wrap;
    gap: 0.35rem;
  }
  .emede-property-chip {
    ...
    border-radius: 999px;
    color: var(--color-link);
    background: color-mix(in srgb, var(--color-link) 10%, var(--color-bg));
    font-family: var(--font-code);
    font-size: 0.82em;
    line-height: 1.45;
  }
  @media (max-width: 34rem) {
    .emede-property-row {
      grid-template-columns: 1.2rem minmax(4.8rem, 6.5rem) minmax(0, 1fr);
      gap: 0.4rem;
    }
  }
